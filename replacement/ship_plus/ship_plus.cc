...
ship_plus::ship_plus(CACHE* cache)
    : replacement(cache), NUM_SET(cache->NUM_SET), NUM_WAY(cache->NUM_WAY), sampler(SAMPLER_SET_FACTOR * NUM_CPUS * static_cast<std::size_t>(NUM_WAY)),
      rrpv_values(static_cast<std::size_t>(NUM_SET * NUM_WAY), maxRRPV), block_hit(static_cast<std::size_t>(NUM_SET * NUM_WAY), 0)
{
...
int& ship_plus::get_rrpv(long set, long way) { return rrpv_values.at(static_cast<std::size_t>(set * NUM_WAY + way)); }
int& ship_plus::get_block_hit(long set, long way) { return block_hit.at(static_cast<std::size_t>(set * NUM_WAY + way)); }
// find replacement victim
long ship_plus::find_victim(uint32_t triggering_cpu, uint64_t instr_id, long set, const champsim::cache_block* current_set, champsim::address ip,
...
void ship_plus::update_replacement_state(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip,
                                    champsim::address victim_addr, access_type type, uint8_t hit)
{
  ...
  if (hit) {
    get_rrpv(set, way) = 0;
    get_block_hit(set,way) = 1;
  }
  else {
    get_block_hit(set, way) = 0;
    // SHIP prediction
